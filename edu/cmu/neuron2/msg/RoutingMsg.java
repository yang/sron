...
public class RoutingMsg extends BaseMsg {
	...
	public RoutingMsg(int identifier, ArrayList<Integer> ml,
			ArrayList<Integer> pt) {
		...
		/*
		 * for (int i = 0; i < numNodes; i++) { probeTable[i] = -1; }
		 */
	}
	...
	/*
	 * public void populateProbeTable(int[] pt) { for (int i = 0; i < numNodes;
	 * i++) { probeTable[i] = pt[i]; } }
	 * 
	 * public void getProbeTable(int[] p) { for (int i = 0; i < numNodes; i++) {
	 * p[i] = probeTable[i]; } }
	 * 
	 * public int getProbeEntry(int offset) { if ((offset >= 0) && (offset <
	 * numNodes)) { return probeTable[offset]; } return -1; }
	 */
