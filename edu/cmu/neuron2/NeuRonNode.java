...
public class NeuRonNode extends Thread implements IRonNode {
	...
	int[][] probeTable; // probeTable[i] = node members[i]'s probe table. value at index j in row i is the link latency between nodes members[i]->members[j].
			...
			try {
				...
				while (true) {
					...
					try {
						...
						System.out.println(this.iNodeId + "couldn't connect, retrying in 1 sec");
						Thread.sleep(1000);
					}
				}
